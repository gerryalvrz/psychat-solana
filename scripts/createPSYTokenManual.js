...
async function createPSYToken() {
  ...
  try {
    ...
    console.log(`🔑 Private key: ${Buffer.from(payer.secretKey).toString('base64')}`);
    ...
    if (balance < 0.1) {
      console.log('\n⚠️  Insufficient SOL balance for token creation.');
      console.log('📝 Please airdrop SOL to this wallet:');
      console.log(`🔗 Wallet Address: ${payer.publicKey.toString()}`);
      console.log('\n💡 You can use:');
      console.log('   - Solana CLI: solana airdrop 2 <address> --url devnet');
      console.log('   - SolFaucet: https://faucet.solana.com/');
      console.log('   - Or any other devnet faucet');
      
      await waitForUserInput('\n⏳ Press Enter after you have funded the wallet...');
      
      // Check balance again
      balance = await checkBalance(connection, payer.publicKey);
      console.log(`💰 Updated balance: ${balance} SOL`);
      
      if (balance < 0.1) {
        throw new Error('Insufficient SOL balance. Please fund the wallet and try again.');
      }
    }
    ...
    console.log(`- Private Key (Base64): ${Buffer.from(payer.secretKey).toString('base64')}`);
